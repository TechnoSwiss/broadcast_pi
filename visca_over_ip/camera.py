...
class Camera:
    ...
    def __init__(self, ip: str, port=52381):
        ...
        self._sock.bind(('', port)) # Commit 513cd8d of VISCA-IP-Controller changes this to bind to local port and query to get actualy port number to allow multiple cameras to be active at the same time, that change prevented query responses for some reason
    ...
    def _send_command(self, command_hex: str, query=False) -> Optional[bytes]:
        ...
        for retry_num in range(self.num_retries):
            ...
            # random delay added between retries to attempt to reduce the number of failures on retry
    ...
    def zoom_to(self, position_int: int): # VISCA-IP-Controller has this set as a float, for some reason using floats is breaking my implementation so leave as int (values sent are integer anyway)
        """Zooms to an absolute position

        :param position: 0-1, where 1 is zoomed all the way in
        """
        #position_int = round(position * 16384) # VISCA-IP-Controller has this set as a float, for some reason using floats is breaking my implementation so leave as int (values sent are integer anyway)
    ...
    def recall_preset(self, preset_num: int):
        ...
        if not 0 <= preset_num <= 255:
            raise ValueError('Preset number must be 0-255 inclusive')
        self._send_command(f'04 3F 02 0{preset_num:02x}')
    ...
    def get_pantilt_position(self) -> Tuple[int, int]:
        ...
        pan_bytes = response[1:5] # Commit 426e7ff of VISCA-IP-Controller changes this to pan_bytes = response[2:6] which offsets the byte array from the correct value by 1, not sure why this change was made
        tilt_bytes = response[5:9] # Commit 426e7ff of VISCA-IP-Controller changes this to tilt_bytes = response[6:10] which offsets the byte array from the correct value by 1, not sure why this change was made
